...
export function WeeklyLite({ onAddToCart, onBuyNow }: WeeklyLiteProps) {
  const { getProductVariants, products } = useAdminData();
  const actualProduct = products.find(p => p.id === 'weekly-lite');
  const packages = getProductVariants('weekly-lite');
  ...
  return (
    <div className="w-full min-h-screen pt-24 pb-12 px-margin-mobile md:px-margin-desktop">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <img src="https://images.unsplash.com/photo-1552820728-8b83bb6b773f?auto=format&fit=crop&q=80" alt="Weekly Lite" className="w-16 h-16 rounded-none object-cover filter sepia-[0.3] hue-rotate-[-30deg] saturate-[1.5]" />
          <div>
            <h1 className="font-headline-md text-on-surface">Weekly Lite {'{'} BD {'}'}</h1>
