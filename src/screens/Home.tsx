...
function FeaturedSection({
  ...
  const currentGame =
    gamesList && gamesList[currentIndex] ? gamesList[currentIndex] : null;
    ...
    <div className="w-full flex flex-col md:flex-row gap-4 h-auto md:h-[500px]">
      ...
      <div className="md:w-2/3 h-[400px] md:h-full relative overflow-hidden tech-clip shadow-lg border border-primary/20 group cursor-pointer bg-surface">
        ...
        <div className="absolute inset-0 p-8 flex flex-col justify-end text-left z-10 pointer-events-none">
          <span className="font-label-sm text-primary uppercase tracking-widest mb-2">
            {currentGame.tagline}
          </span>
          <h2 className="font-display-md md:font-display-lg text-on-surface mb-3 leading-tight max-w-md">
            {currentGame.title}
          </h2>
          ...
          <div className="flex items-center gap-4 pointer-events-auto">
            <span className="font-headline-md text-on-surface mr-2">
              {currentGame.price === 0 ? "Free" : `$${currentGame.price}`}
            </span>
            <motion.button
            ...
            <button
              ...
              className="w-12 h-12 bg-surface-container/50 tech-clip-btn border border-white/10 flex items-center justify-center text-primary hover:bg-white/10 transition-colors backdrop-blur-md"
            >
              <Zap size={18} />
            </button>
            <button
          ...
          <div
            ...
            className={`flex items-center gap-4 p-3 tech-clip cursor-pointer transition-colors group ${
              currentIndex === i
                ...
                : "bg-surface-container/30 hover:bg-surface-container/80"
            }`}
          >
            <img
              src={game.image}
              className="w-16 h-20 object-cover shadow-sm tech-clip"
              alt={game.title}
            />
            <div className="flex-1 flex flex-col items-start text-left">
              <h3
                className={`font-label-md line-clamp-1 mb-1 transition-colors ${currentIndex === i ? "text-primary" : "text-on-surface-variant group-hover:text-on-surface"}`}
              >
                {game.title}
              </h3>
...
export function Home({
  ...
  const { settings, homeSections, products, error } = useAdminData();

  const currentFeaturedGames =
  ...
  const handleCategoryClick = (title: string, e: React.MouseEvent) => {
    ...
    if (title === "Game Top-Ups") {
      ...
      const netflix = products.find(
        (p) => p.id === "netflix" || p.title?.toLowerCase().includes("netflix"),
      );
      if (netflix) {
        onProductSelect({
          id: netflix.id,
      ...
      const discord = products.find(
        (p) =>
          p.id === "discord-nitro" ||
          p.title?.toLowerCase().includes("discord"),
      );
      if (discord) {
        onProductSelect({
          id: discord.id,
  ...
  return (
    <div className="flex-grow flex flex-col pt-20">
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 text-red-400 p-4 mx-4 mt-4 rounded-lg text-sm">
          <strong>Firebase Permissions Error:</strong> {error}
          <br />
          Please update your Firestore security rules in your Firebase Console
          to allow reading (and optionally writing) during development. For
          example: <code>allow read, write: if true;</code>
        </div>
      )}
      {/* Hero Section */}
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden px-margin-mobile md:px-margin-desktop py-24">
        ...
        >
          <FeaturedSection
            featuredGames={currentFeaturedGames}
      ...
      {(() => {
        ...
        return (
          <section
            id="top-up-section"
            className="py-16 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto w-full"
          >
            <h2 className="font-headline-md text-primary text-glow mb-10 text-center">
              Top-Up
            </h2>
            <div className="flex flex-wrap justify-center gap-8 md:gap-16">
              {sortedTopUp.map((item, i) => (
                <motion.div
                  ...
                  onClick={() => {
                    onProductSelect({
                      ...
                      description:
                        item.description ||
                        `Purchase ${item.title} securely with instant delivery.`,
                ...
                >
                  ...
                  <h3 className="font-label-md text-on-surface group-hover:text-primary transition-colors text-center max-w-[160px]">
                    {item.title}
                  </h3>
                </motion.div>
              ))}
          ...
          {[
            {
              icon: Gamepad2,
              title: "Game Top-Ups",
              desc: "Instant delivery for major titles.",
            ...
            },
            {
              icon: Tv,
              title: "Streaming",
              desc: "Premium entertainment access.",
              color: "secondary",
              shadow: "#ff005560",
            },
            {
              icon: Key,
              title: "Software Keys",
              desc: "OS and productivity licenses.",
              color: "primary-container",
              shadow: "#00ffcc60",
            },
            {
            ...
            >
              <div
                className={`w-12 h-12 tech-clip bg-${cat.color}/20 flex items-center justify-center text-${cat.color} mb-4 group-hover:scale-110 transition-transform`}
              >
                <cat.icon />
              </div>
              <h3 className="font-headline-md text-body-lg text-on-surface mb-2">
                {cat.title}
              </h3>
      ...
      {Array.from(new Set(products.map((p) => p.folder || p.category)))
        ...
        .map((sectionTitle, idx) => {
          ...
          const sectionProducts = products.filter(
            (p) => (p.folder || p.category) === titleStr,
          );
          if (sectionProducts.length === 0) return null;

          return (
            <section
              id={`section-${(titleStr || "").toLowerCase().replace(/\s+/g, "-")}`}
              key={titleStr || idx}
              className="py-16 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto w-full"
            >
              <h2 className="font-headline-md text-primary text-glow mb-10 text-center">
      ...
      {homeSections.map((section, idx) => (
        <section
          key={section.id || idx}
          className={`${idx === 0 ? "py-16" : "pb-16"} px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto w-full flex-grow`}
        >
          <h2 className="font-headline-md text-primary text-glow mb-10">
            {section.title}
          </h2>
              ...
              <motion.div
                ...
                onClick={() =>
                  onProductSelect({
                    id: prod.title,
                    title: prod.title,
                    description: prod.description || prod.title + " details.",
                    price: prod.price,
                    image: prod.image,
              ...
              >
                <div className="aspect-square w-full bg-surface-container rounded-2xl relative overflow-hidden mb-3">
                  ...
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleFavorite(prod.title, e);
                    }}
                  >
                    <Heart
                      size={16}
                      fill={isFavorite(prod.title) ? "currentColor" : "none"}
                      className={isFavorite(prod.title) ? "text-red-500" : ""}
                    />
                  </div>
                  ...
                  <div className="flex gap-2">
                    {(() => {
                      const mainProd = products.find(
                        (p) => p.title === prod.title,
                      );
                      const isOutOfStock =
                        mainProd &&
                        mainProd.variants &&
                        mainProd.variants.length > 0 &&
                        !mainProd.variants.some((v) => v.stock === "In Stock");
                      if (isOutOfStock) {
                        return (
      ...
      <section className="py-16 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto w-full">
        ...
        <div className="flex flex-wrap justify-center gap-8 md:gap-16">
          {[
            {
              title: "Facebook Page Like/Followers",
              image:
            ...
            },
            {
              ...
              image:
                "https://images.unsplash.com/photo-1611605698335-8b1569810432?auto=format&fit=crop&q=80",
            },
            {
            ...
            >
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl overflow-hidden relative tech-clip shadow-[0_0_20px_rgba(0,240,255,0.15)] group-hover:shadow-[0_0_30px_rgba(0,240,255,0.4)] transition-all duration-300 border border-primary/30 group-hover:border-primary">
                <div className="absolute top-2 right-2 bg-black/80 text-white font-bold text-[10px] px-2 py-0.5 rounded shadow-sm z-20">
                  Ads
                </div>
                <div className="absolute inset-0 bg-primary/10 mix-blend-multiply z-10 group-hover:bg-transparent transition-colors" />
                <img
