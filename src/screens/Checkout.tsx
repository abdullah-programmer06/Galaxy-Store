...
export function Checkout({
  ...
  onClearCart,
  onNavigate,
}: CheckoutProps & { onNavigate?: (view: any) => void }) {
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-[#0a0616] pt-24 pb-12 flex items-center justify-center">
        <div className="bg-surface-container border border-white/10 rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <button
            ...
            className="w-full bg-primary text-on-primary font-bold py-3 rounded-lg hover:bg-primary-container transition-colors"
          >
            Go to Login
          </button>
        </div>
    ...
    return (
      <div className="pt-24 pb-12 px-margin-mobile md:px-margin-desktop max-w-container-md mx-auto min-h-[70vh] flex flex-col justify-center items-center text-center">
        ...
        >
          ...
          <h2 className="font-display-sm text-on-surface mb-2">
            Order Placed Successfully!
          </h2>
          <p className="font-body-md text-on-surface-variant mb-8">
            Thank you for your order. We are verifying your payment. Your
            digital code or top-up will be delivered shortly.
          </p>
          <motion.button
  ...
  return (
    <div className="pt-24 pb-12 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
      <div className="mb-8">
        <h1 className="font-display-md text-on-surface tracking-tight mb-2">
          Checkout
        </h1>
            ...
            <form onSubmit={handleCheckout} className="space-y-6">
              ...
              <div>
                <h2 className="font-headline-md text-on-surface mb-4 flex items-center gap-2">
                  <User className="text-primary" size={20} /> Personal
                  Information
                </h2>
              ...
              <div className="border-t border-white/10 pt-6">
                <h2 className="font-headline-md text-on-surface mb-4 flex items-center gap-2">
                  <Wallet className="text-primary" size={20} /> Choose Payment
                  Method
                </h2>
                ...
                {paymentMethod && (
                  <div className="bg-surface-container/50 border border-white/5 p-4 md:p-6 mb-6">
                    {(() => {
                      const activeMethod = settings?.paymentMethods.find(
                        (m) => m.method === paymentMethod,
                      );
                      if (!activeMethod) return null;

                      const actionText =
                        activeMethod.type === "Personal"
                          ? '"Send Money"'
                          : activeMethod.type === "Agent"
                            ? '"Cash Out"'
                            : '"Payment"';
                      ...
                      return (
                        <>
                          <p className="text-sm text-on-surface mb-3">
                            Please send the total amount{" "}
                            <span className="text-primary font-bold">
                              ৳{finalTotal.toFixed(2)}
                            </span>{" "}
                            as{" "}
                            <span className="font-semibold font-bold text-primary">
                              {actionText}
                            </span>{" "}
                            to our {activeMethod?.type?.toLowerCase()} number:
                          </p>
                          <div className="bg-surface-container-high p-3 flex justify-between items-center border border-white/10 mb-4 font-mono text-lg text-white font-bold group relative">
                            ...
                            <span className="text-xs text-primary bg-primary/10 px-2 py-1 font-sans rounded uppercase">
                              {activeMethod.type}
                            </span>
                          </div>
                        </>
                    ...
                    <div className="mb-4">
                      <label className="block font-label-sm text-on-surface-variant mb-1">
                        আপনার পেমেন্টের নাম্বারটি এড করুন (যেটা দিয়ে আপনি
                        পেমেন্ট করেছেন)
                      </label>
                      <input
                        type="text"
          ...
          <div className="glass-panel tech-clip p-6 md:p-8 sticky top-28">
            ...
            <div className="space-y-4 mb-6 max-h-[40vh] overflow-y-auto pr-2">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="flex justify-between items-center bg-surface-container/50 p-3 rounded-none border border-white/5"
                >
                  <span className="font-label-sm text-on-surface line-clamp-1 flex-grow pr-4">
                    {item.title}
                  </span>
                  <span className="font-body-md text-primary flex-shrink-0">
