...
export function AdminDashboard() {
  ...
  const [productCategoryFilter, setProductCategoryFilter] = useState<
    "All" | "Top-Up" | "Games" | "Software" | "Subscriptions"
  >("All");
  ...
  const [isAddingSectionProduct, setIsAddingSectionProduct] = useState<
    string | null
  >(null);
  const [newSectionProduct, setNewSectionProduct] = useState({
    title: "",
    type: "Top-Up",
    price: 0,
    image: "",
  ...
  const [managingSectionId, setManagingSectionId] = useState<string | null>(
    null,
  );

  const [newStaffEmail, setNewStaffEmail] = useState("");
  const [newStaffRole, setNewStaffRole] = useState<"Admin" | "Moderator">(
    "Moderator",
  );

  // Featured games slider states
  const [editingSliderIndex, setEditingSliderIndex] = useState<number | null>(
    null,
  );
  ...
  const [editingPaymentMethods, setEditingPaymentMethods] = useState<
    PaymentMethodConfig[]
  >([]);
  ...
  const staffMember = settings?.staff?.find(
    (s) => s.email?.toLowerCase() === currentUser?.email?.toLowerCase(),
  );
  const isSuperAdmin =
    currentUser?.email?.toLowerCase() === "abubokkorbaqi@gmail.com";
  ...
  const handleAddVariant = () => {
    ...
    setEditingVariants([
      ...editingVariants,
      { id: newId, name: "New Variant", price: 0, stock: "In Stock" },
    ]);
  };
      ...
      <div className="bg-surface-container border border-primary/30 rounded-xl overflow-hidden mt-4 shadow-lg shadow-primary/5">
        ...
        <div className="p-4 space-y-4 border-b border-white/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                type="text"
                value={editingProductDetails?.title || ""}
                onChange={(e) =>
                ...
                className="w-full bg-surface border border-white/10 rounded px-3 py-2 text-white text-sm"
              />
            </div>
            <div>
              <label className="block text-xs text-on-surface-variant mb-1">
              ...
              <input
                type="text"
                value={editingProductDetails?.folder || ""}
                onChange={(e) =>
                  setEditingProductDetails({
                    ...editingProductDetails,
              ...
              <datalist id="edit-folder-options">
                {Array.from(
                  new Set(mockProducts.map((p) => p.folder).filter(Boolean)),
                ).map((folder, idx) => (
            ...
            <div>
              ...
              <input
                type="number"
                value={editingProductDetails?.basePrice || 0}
                onChange={(e) =>
                  setEditingProductDetails({
                    ...editingProductDetails,
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  type="text"
                  value={editingProductDetails?.image || ""}
                  onChange={(e) =>
                    setEditingProductDetails({
                      ...editingProductDetails,
              ...
              {editingProductDetails?.images &&
                editingProductDetails.images.length > 0 && (
                  <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
                    {editingProductDetails.images.map(
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditingProductDetails({
                                ...
                                images: newImages,
                                image:
                                  newImages[0] || editingProductDetails.image,
                              });
                            }}
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="w-full bg-surface border border-white/10 rounded px-3 py-2 text-white text-sm min-h-[100px]"
        ...
        <div className="p-4 space-y-4">
          ...
          {editingVariants.map((variant, index) => (
            <div
              key={variant.id}
              className="grid grid-cols-12 gap-4 items-center bg-white/5 p-2 rounded-lg"
            >
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#0a0616] flex justify-center items-center text-white">
        Loading...
      </div>
    );
  if (!isStaff)
    return (
      <div className="min-h-screen bg-[#0a0616] flex flex-col justify-center items-center text-white p-4 text-center">
        ...
        <p className="text-on-surface-variant max-w-md mt-2">
          If you were recently added as a Moderator, make sure you are signed in
          with the exact email address that was granted access.
        </p>
      </div>
    );
  ...
  const stats = [
    {
      label: "Total Revenue",
      value: `৳${completedOrdersTotal.toFixed(2)}`,
      trend: "+15%",
    ...
    },
    {
      label: "Active Users",
      value: Array.from(
        new Set(orders.map((o: Order) => o.email)),
      ).length.toString(),
      trend: "+12%",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className="bg-surface-container border border-white/5 p-6 rounded-xl"
          >
            ...
            <tbody className="divide-y divide-white/5">
              {recentOrders.map((order, idx) => (
                <tr key={idx} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4">
                    <span
                      className={`px-2 py-1 rounded text-xs font-bold flex items-center gap-1 w-max ${
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          {homeSections.map((section) => (
            <div
              key={section.id}
              className="bg-surface-container border border-white/5 rounded-xl overflow-hidden p-6"
            >
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-on-surface-variant text-sm px-2 py-1 bg-white/5 rounded-full">
                ...
                >
                  <Trash2 size={16} />{" "}
                  {confirmAction === `delete_home_${section.id}`
                    ? "Click to confirm!"
                    : "Delete Section"}
                </button>
              </div>
            ...
            {activeTab === "products"
              ...
              : activeTab === "Software"
                ...
                : [
                  ...
                  ? mockProducts.find((p) => p.id === activeTab)?.title
                  : activeTab}{" "}
            Management
          </h2>
          <div className="flex items-center gap-3">
            {activeTab !== "products" && (
              ...
              >
                <Trash2 size={18} />{" "}
                {confirmAction === `delete_cat_${activeTab}`
                  ? "Click to confirm!"
                  : "Delete Section"}
              </button>
            )}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-surface-container border border-white/5 rounded-xl p-6">
          <p className="text-on-surface-variant mb-6 text-sm">
            ...
            Ensure URLs are complete (e.g. https://wa.me/..., mailto:...,
            https://discord.gg/...).
          </p>
  ...
  const renderSettings = () => {
    ...
    const presetLogos = [
      {
        name: "bKash Logo",
        url: "https://upload.wikimedia.org/wikipedia/commons/d/df/BKash_Logo.svg",
      },
      {
        name: "Nagad Logo",
        url: "https://upload.wikimedia.org/wikipedia/commons/8/80/Nagad_Logo.svg",
      },
      {
        name: "Rocket Logo",
        url: "https://upload.wikimedia.org/wikipedia/commons/d/d1/Rocket_by_Dutch-Bangla_Bank_Logo_transparent.png",
      },
    ];
      ...
      <div className="space-y-6">
        ...
        <div className="bg-surface-container border border-white/5 rounded-xl p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <div>
              ...
              <p className="text-sm text-on-surface-variant mt-1">
                Configure bKash, Nagad, Rocket, or other payment gateways with
                their respective account details.
              </p>
            </div>
            <button
              type="button"
          ...
          {isSavedPayment && (
            ...
            >
              <CheckCircle size={16} /> Payment settings updated successfully in
              database!
            </motion.div>
          )}
          ...
          <div className="space-y-4 mt-6">
            {editingPaymentMethods.map((method, index) => (
              <div
                key={index}
                className="p-4 bg-white/5 border border-white/10 rounded-xl space-y-4 relative"
              >
                <div className="flex justify-between items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={method.logo || ""}
                      onChange={(e) =>
                        handleUpdatePaymentMethod(index, "logo", e.target.value)
                      }
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-1 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    {presetLogos.map((preset) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleUpdatePaymentMethod(index, "logo", preset.url)
                        }
                    ...
                    {method.logo && (
                      <button
                        type="button"
                        onClick={() =>
                          handleUpdatePaymentMethod(index, "logo", "")
                        }
                  ...
                  {method.logo && (
                    <div className="flex items-center gap-2 bg-[#0c081d] border border-white/10 rounded px-2 py-1">
                      <span className="text-on-surface-variant font-semibold text-[10px]">
                        Logo Preview:
                      </span>
                      <img
                        src={method.logo}
            ...
            {editingPaymentMethods.length === 0 && (
              <div className="p-8 text-center text-on-surface-variant border border-dashed border-white/10 rounded-xl">
                No active payment methods configured. Click "Add Payment Method"
                to configure bKash, Nagad, or Rocket.
              </div>
            )}
          </div>
  ...
  const handleSaveSliderItem = async (e: React.FormEvent) => {
    e.preventDefault();
    const currentList =
      settings?.featuredGames && settings.featuredGames.length > 0
  ...
  const handleMoveSliderItem = async (
    index: number,
    direction: "up" | "down",
  ) => {
    const currentList =
  ...
  const renderFeaturedSlider = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-on-surface-variant text-sm mt-1">
              Customize the interactive cards and sliders displayed at the very
        ...
        {isAddingSliderItem ? (
          <form
            onSubmit={handleSaveSliderItem}
            className="bg-surface-container border border-white/5 rounded-xl p-6 space-y-4"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSliderForm({ ...sliderForm, tagline: e.target.value })
                  }
            ...
            {currentList.length === 0 ? (
              ...
              <div className="divide-y divide-white/5">
                {currentList.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-white/[0.01] transition-colors"
                  >
                    <div className="flex items-center gap-4">
                      <div className="text-on-surface-variant font-mono text-sm w-4 text-center">
                        {idx + 1}
                      </div>
                      <img
                        src={item.image}
                        className="w-12 h-16 object-cover rounded border border-white/10 bg-black flex-shrink-0"
                        alt=""
                      />
                      <div className="text-left">
                        ...
                        <span className="text-primary text-xs font-semibold mt-1 block">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      ...
                      >
                        <Trash2 size={16} />
                        {confirmAction === `delete_slider_${idx}` && (
  ...
  if (!isStaff)
    return (
      <div className="min-h-screen bg-[#0a0616] flex justify-center items-center text-red-500 font-bold text-xl">
        Access Denied
      </div>
    );
  ...
  return (
    <>
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 text-red-400 p-4 mx-4 mt-20 rounded-lg text-sm max-w-[1400px] w-full mx-auto relative z-50">
          <strong>Firebase Permissions Error:</strong> {error}
          <br />
          Please update your Firestore security rules in your Firebase Console
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === id ? "bg-primary/20 text-primary border border-primary/30" : "text-on-surface-variant hover:bg-white/5"}`}
                  >
                    <Package size={18} />
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {[
                  ...
                  "level-up-pass",
                  ...Array.from(
                    new Set(mockProducts.map((p) => p.folder || p.title)),
                  ),
                ].includes(activeTab) && renderProducts()}
                {activeTab === "orders" && (
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      <h2 className="text-2xl font-bold text-white">
                            ...
                            <tbody className="divide-y divide-white/5">
                              {orders.map((order: Order) => (
                                ...
                                >
                                  ...
                                  <td className="p-4 max-w-xs">
                                    <div className="text-white text-xs space-y-1">
                                      {order.items.map((item, idx) => (
                                        <div
                                          key={idx}
                                          className="bg-white/5 px-2 py-1 rounded border border-white/5 font-medium"
                                        >
                                          {item.title}
                                        </div>
                                      ))}
                                  ...
                                  <td className="p-4">
                                    <span
                                      className={`px-2.5 py-1 rounded text-xs font-bold flex items-center gap-1.5 w-max ${
                                        order.status === "Completed"
                                          ? "bg-green-500/20 text-green-500"
                                          : order.status === "Pending"
                                            ? "bg-yellow-500/20 text-yellow-500"
                    ...
                    {isSuperAdmin ? (
                      <div className="bg-surface-container border border-white/5 rounded-xl p-6">
                        ...
                        <p className="text-sm text-yellow-500/90 mb-4 bg-yellow-500/10 p-3 rounded border border-yellow-500/20">
                          <strong>Important:</strong> Adding an email here only
                          grants permission. The user must independently "Sign
                          Up" for an account on the website using this email
                          address to log in.
                        </p>
                          ...
                          {settings?.staff?.map((staff) => (
                            ...
                            >
                              <div>
                                ...
                                <span
                                  className={`text-xs font-mono uppercase px-2 py-1 rounded ${staff.role === "Admin" ? "bg-primary/20 text-primary" : "bg-blue-500/20 text-blue-400"}`}
                                >
                                  {staff.role}
                                </span>
                              </div>
                              <button
                                onClick={() => {
                                  if (staff.email === "abubokkorbaqi@gmail.com")
                                    return; // Cannot delete superadmin
                                  const newStaff = settings.staff.filter(
                                    (s) => s.email !== staff.email,
                                  );
                                  updateSettings({ staff: newStaff });
                                }}
                                disabled={
                                  staff.email === "abubokkorbaqi@gmail.com"
                                }
                ...
                >
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-white">
                      Manage Products in{" "}
                      {
                        homeSections.find((s) => s.id === managingSectionId)
                          ?.title
                      }
                    </h3>
                    <button
                  ...
                  <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                    ...
                    {homeSections
                      .find((s) => s.id === managingSectionId)
                      ?.products.map((prod) => (
                        <div
                          key={prod.id}
                          className="grid grid-cols-12 gap-4 items-center bg-white/5 p-2 rounded-lg"
                        >
                          <div className="col-span-4 text-white text-sm truncate">
                            {prod.title}
                          </div>
                          ...
                          <div className="col-span-2 truncate text-xs text-on-surface-variant">
                            {prod.image}
                          </div>
                          <div className="col-span-1 flex justify-center">
                            <button
                  ...
                  <div className="mt-6 pt-4 border-t border-white/10">
                    <button
                      onClick={() =>
                        setIsAddingSectionProduct(managingSectionId)
                      }
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <datalist id="folder-options">
                          {Array.from(
                            new Set(
                              mockProducts.map((p) => p.folder).filter(Boolean),
                            ),
                          ).map((folder, idx) => (
                            <option key={idx} value={folder} />
                          ))}
                    ...
                    <div>
                      ...
                      {(newProduct as any).images &&
                        (newProduct as any).images.length > 0 && (
                          <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
                            {(newProduct as any).images.map(
                              (img: string, idx: number) => (
                                <div
                                  key={idx}
                                  className="relative w-12 h-12 flex-shrink-0 group"
