...
export function UIDTopUp({ onAddToCart, onBuyNow }: UIDTopUpProps) {
  const { getProductVariants, products } = useAdminData();
  const actualProduct = products.find(p => p.id === 'uid-topup');
  const packages = getProductVariants('uid-topup');
  ...
  return (
    <div className="w-full min-h-screen pt-24 pb-12 px-margin-mobile md:px-margin-desktop">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <img src="https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80" alt="UID TopUp" className="w-16 h-16 rounded-none object-cover filter sepia-[0.3] hue-rotate-[-30deg] saturate-[1.5]" />
          <div>
            <h1 className="font-headline-md text-on-surface">UID TopUp {'{'} BD {'}'}</h1>
                ...
                {packages.map((pkg: any) => (
                  ...
                  >
                    <span className="font-label-md text-on-surface flex items-center gap-1">
                      {pkg.name} 
                      {pkg.name?.toLowerCase().includes('diamond') ? <span className="text-primary text-lg">💎</span> : <span className="text-secondary text-lg">🎟️</span>}
                    </span>
                    <span className="font-body-sm text-primary">BDT {pkg.price}</span>
