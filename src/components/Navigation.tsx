import { ShoppingCart, User, X, Box, Trash2, Menu, Search, Settings, LogOut, LogIn } from 'lucide-react';
...
export function Navigation({ currentRoute, onNavigate, cartItems, onRemoveFromCart, onProductSelect }: NavigationProps) {
      ...
      <header className="fixed top-0 w-full z-50 border-b border-white/10 bg-surface/60 backdrop-blur-xl">
        <div className="flex justify-between items-center h-20 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
          ...
          <div className="hidden md:flex relative flex-1 max-w-sm mx-6">
            ...
            <AnimatePresence>
              {isSearchFocused && searchQuery.length > 0 && (
                ...
                >
                  {allProducts.filter(p => p.title?.toLowerCase().includes(searchQuery?.toLowerCase())).slice(0, 5).map(product => (
                    <div 
                      key={product.id}
                      onClick={() => {
                        setSearchQuery("");
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-hidden">
                        <div className="text-sm text-white font-bold truncate">{product.title}</div>
                        <div className="text-xs text-primary truncate">{product.category}</div>
                      </div>
                      <div className="text-sm text-white font-bold">৳{product.price}</div>
                    </div>
                  ))}
                  {allProducts.filter(p => p.title?.toLowerCase().includes(searchQuery?.toLowerCase())).length === 0 && (
                    <div className="p-4 text-sm text-on-surface-variant text-center">No products found.</div>
                  )}
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="flex items-center gap-4 text-primary">
            ...
            {(user?.email?.toLowerCase() === 'abubokkorbaqi@gmail.com' || settings?.staff?.some(s => s.email?.toLowerCase() === user?.email?.toLowerCase())) && (
              <button onClick={() => onNavigate('admin')} className="hidden sm:block hover:bg-white/5 p-2 rounded-none transition-all duration-300 active:scale-95 text-on-surface-variant hover:text-primary" title="Admin Dashboard">
              <Settings size={24} />
            </button>
            )}
            <button 
              ...
              <div className="px-4 pt-4 md:hidden">
                ...
                {searchQuery.length > 0 && (
                  <div className="mt-2 flex flex-col bg-surface-container-high rounded-md border border-white/5 overflow-hidden">
                    {allProducts.filter(p => p.title?.toLowerCase().includes(searchQuery?.toLowerCase())).slice(0, 4).map(product => (
                      <div 
                        key={product.id}
                        onClick={() => {
                          setSearchQuery("");
                ...
                {navLinks.map((link) => (
                  <button
                    ...
                    className={`text-left font-headline-md text-headline-md p-2 rounded ${
                      currentRoute === link.route ? 'text-primary bg-white/5' : 'text-on-surface-variant'
                    }`}
                  >
                    {link.label}
                  </button>
                ...
                <>
                  ...
                  <button
                    ...
                    className={`text-left font-headline-md text-headline-md p-2 rounded flex items-center gap-2 border-t border-white/10 mt-2 pt-4 ${
                      currentRoute === 'admin' ? 'text-primary bg-white/5' : 'text-on-surface-variant hover:text-primary'
                    }`}
                  >
                    <Settings size={20} className="text-primary animate-pulse" />
            ...
            >
              ...
              <div className="flex-grow p-6 overflow-y-auto flex flex-col gap-4">
                {cartItems.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center text-on-surface-variant opacity-50">
                    ...
                    <p className="font-body-md text-body-md">Your arsenal is empty.</p>
                  </div>
                ) : (
                  cartItems.map(item => (
                    <motion.div 
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <h4 className="font-label-sm text-label-sm text-on-surface line-clamp-1">{item.title}</h4>
                        <span className="font-body-md text-body-md text-primary">${item.price.toFixed(2)}</span>
                      </div>
                      <button 
              ...
              <div className="p-6 border-t border-white/10 bg-surface-container-lowest/50">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="font-headline-md text-headline-md text-primary">${cartTotal.toFixed(2)}</span>
                </div>
                <motion.button 
