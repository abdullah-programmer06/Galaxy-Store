...
export function TopUpDetails({
  ...
  const packages = getProductVariants(product.id) || [];
  ...
  const checkPlayerName = async () => {
    ...
    try {
      const response = await fetch(
        `/api/player-name?uid=${encodeURIComponent(playerId.trim())}`,
      );
      ...
      if (data && data.error === false && data.data && data.data.username) {
        ...
        setNameError(
          data.msg === "id_not_found"
            ? "প্লেয়ার আইডি পাওয়া যায়নি"
            : "আইডি চেক করতে ব্যর্থ হয়েছে। আবার চেষ্টা করুন।",
        );
      }
    } catch (err) {
  ...
  const handleBuy = () => {
    if (selectedPackage && playerId) {
      ...
      if (pkg) {
        onBuyNow({
          title: `${actualProduct?.title || "Free Fire"} ${pkg.name} (UID: ${playerId})`,
          price: pkg.price,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <img
            ...
            alt={actualProduct?.title || "TopUp"}
            className="w-16 h-16 rounded-none object-cover filter sepia-[0.3] hue-rotate-[-30deg] saturate-[1.5]"
          />
          <div>
            <h1 className="font-headline-md text-on-surface">
              {actualProduct?.title || "TopUp"}
            </h1>
          ...
          >
            <div className="glass-panel tech-clip border border-white/10 p-6">
              <div className="flex items-center gap-2 mb-6 border-b border-white/5 pb-4">
                <div className="w-8 h-8 rounded-none bg-primary/20 flex items-center justify-center text-primary font-bold">
                  1
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
                {packages.map((pkg: any) => (
                  <button
                    key={pkg.id}
                    onClick={() =>
                      pkg.stock !== "Out of Stock" && setSelectedPackage(pkg.id)
                    }
                  ...
                  >
                    ...
                    {pkg.stock === "Out of Stock" && (
                      <span className="absolute top-1 right-1 text-[9px] text-red-500 font-bold uppercase">
                        Out of Stock
                      </span>
                    )}
                  </button>
              ...
              <div className="mt-6 pt-4 border-t border-white/5">
                <a
                  href="#"
                  className="text-secondary hover:text-primary transition-colors font-label-sm flex items-center gap-1"
                >
            ...
            <div className="glass-panel tech-clip border border-white/10 p-6">
              ...
              <div className="space-y-4">
                <label className="block font-label-sm text-on-surface-variant">
                  এখানে আপনার গেমের আইডি কোড লিখুন
                </label>
                <input
                  type="text"
                  value={playerId}
            ...
            <div className="glass-panel tech-clip border border-white/10 p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <button
                  ...
                  className={`${selectedPaymentOption === "wallet" ? "bg-primary/10 border-2 border-primary" : "bg-surface-container/50 border border-white/10 hover:border-primary/50"} rounded-none p-4 flex flex-col items-center gap-2 relative transition-all cursor-pointer`}
                >
                  {selectedPaymentOption === "wallet" && (
                    <div className="absolute top-0 left-0 bg-primary text-on-primary w-6 h-6 rounded-none flex items-center justify-center">
                      <CheckCircle2 size={14} />
                    </div>
                  )}
                  <div className="w-16 h-10 bg-white/10 rounded flex items-center justify-center text-xl">
                    💳
                  </div>
                  <span className="font-label-sm text-on-surface text-center">
                    Wallet Pay
                  </span>
                </button>
                <button
                  onClick={() => setSelectedPaymentOption("instant")}
                  className={`${selectedPaymentOption === "instant" ? "bg-primary/10 border-2 border-primary" : "bg-surface-container/50 border border-white/10 hover:border-primary/50"} rounded-none p-4 flex flex-col items-center gap-2 relative transition-all cursor-pointer`}
                >
                  {selectedPaymentOption === "instant" && (
                    <div className="absolute top-0 left-0 bg-primary text-on-primary w-6 h-6 rounded-none flex items-center justify-center">
                      <CheckCircle2 size={14} />
                    </div>
                  )}
              ...
              <div className="space-y-2 mb-6 text-sm">
                <p className="text-on-surface-variant flex items-center gap-2">
                  <Info size={14} /> প্রোডাক্টটি কিনতে আপনার প্রয়োজন{" "}
        ...
        >
          ...
          <div className="space-y-4 text-on-surface-variant font-body-sm leading-relaxed">
            <p className="flex items-start gap-2">
              ...
              />
              শুধুমাত্র Bangladesh সার্ভারের Player ID/UID দিয়ে Top Up করা যাবে।
              অন্য সার্ভারের Player ID/UID হলে অর্ডার করবেন না। Diamond নিতে
              শুধু মাত্র আপনার Player ID Code/UID লাগবে
            </p>
            <p className="flex items-start gap-2">
              <CheckCircle2
                size={16}
            ...
            <p className="flex items-start gap-2">
              ...
              ইভেন্ট টাইমে সর্বোচ্চ ৫ মিনিট সময় লাগতে পারে।
            </p>
            <p className="flex items-start gap-2">
              <CheckCircle2
                size={16}
                className="text-primary mt-1 flex-shrink-0"
              />
              ২-৫ মিনিটের মধ্যে ডেলিভারি না পেলে বা যেকোনো প্রয়োজনে ওয়েবসাইটের
              নিচে ডানপাশে থাকা লাইভ চ্যাটে যোগাযোগ করুন। অথবা আমাদের ফেসবুক
              পেজে মেসেজ করুন আমাদের Support-Team আপনাকে সাহায্য করবে।
            </p>

            <div className="mt-8 pt-4">
              <p className="font-label-md text-on-surface mb-4">
                Player ID/UID কিভাবে পাবেন ?
              </p>
